@256
D=A
@SP
M=D
@Sys.init$RETURN_ADDRESS_0
D=A
@SP
AM=M+1
A=A-1
M=D // top=return address
@LCL
D=M // D=local base
@SP
AM=M+1
A=A-1
M=D // top=local base
@ARG
D=M // D=arg base
@SP
AM=M+1
A=A-1
M=D // top=arg base
@THIS
D=M // D=this base
@SP
AM=M+1
A=A-1
M=D // top=this base
@THAT
D=M // D=that base
@SP
AM=M+1
A=A-1
M=D // top=that base
@0
D=A
@5
D=D+A // D=argCount + 5
@SP
D=M-D // D=stack base - argCount - 5
@ARG
M=D // arg base=stack base - argCount - 5
@SP
D=M
@LCL
M=D // local base=stack base
@Sys.init
0;JMP // jump to function entry
(Sys.init$RETURN_ADDRESS_0)
0
(Class1.set)
@0
D=A
(Class1.set$CHECK_LOCAL_VAR_COUNT)
@Class1.set$LOCAL_VAR_COUNT_ZERO
D;JEQ
@SP
AM=M+1
A=A-1
M=0
D=D-1
@Class1.set$CHECK_LOCAL_VAR_COUNT
0;JMP
(Class1.set$LOCAL_VAR_COUNT_ZERO)
0
@0
D=A
@ARG
A=M
A=D+A
D=M
@SP
A=M
M=D
@SP
M=M+1
@SP
AM=M-1
D=M
@Class1.0
M=D
@1
D=A
@ARG
A=M
A=D+A
D=M
@SP
A=M
M=D
@SP
M=M+1
@SP
AM=M-1
D=M
@Class1.1
M=D
@0
D=A
@SP
A=M
M=D
@SP
M=M+1
@LCL
D=M // D=local base
@5
A=D-A // A=local base - 5(return address base)
D=M // D=return address
@R13
M=D // R13=return address
@SP
AM=M-1
D=M // D=return value
@ARG
A=M
M=D // arg[0]=return value
D=A+1
@SP
M=D // SP base=arg base + 1
@LCL
A=M-1
D=M // D=old THAT
@THAT
M=D // THAT=old THAT
@2
D=A
@LCL
A=M-D
D=M // D=old THIS
@THIS
M=D // THIS=old THIS
@3
D=A
@LCL
A=M-D
D=M // D=old ARG
@ARG
M=D // ARG=old ARG
@4
D=A
@LCL
A=M-D
D=M // D=old LCL
@LCL
M=D // LCL=old LCL
@R13
A=M // A=return address
0;JMP // jump to return address
(Class1.get)
@0
D=A
(Class1.get$CHECK_LOCAL_VAR_COUNT)
@Class1.get$LOCAL_VAR_COUNT_ZERO
D;JEQ
@SP
AM=M+1
A=A-1
M=0
D=D-1
@Class1.get$CHECK_LOCAL_VAR_COUNT
0;JMP
(Class1.get$LOCAL_VAR_COUNT_ZERO)
0
@Class1.0
D=M
@SP
A=M
M=D
@SP
M=M+1
@Class1.1
D=M
@SP
A=M
M=D
@SP
M=M+1
@SP
A=M
A=A-1
D=M
A=A-1
D=M-D
M=D
@SP
M=M-1
@LCL
D=M // D=local base
@5
A=D-A // A=local base - 5(return address base)
D=M // D=return address
@R13
M=D // R13=return address
@SP
AM=M-1
D=M // D=return value
@ARG
A=M
M=D // arg[0]=return value
D=A+1
@SP
M=D // SP base=arg base + 1
@LCL
A=M-1
D=M // D=old THAT
@THAT
M=D // THAT=old THAT
@2
D=A
@LCL
A=M-D
D=M // D=old THIS
@THIS
M=D // THIS=old THIS
@3
D=A
@LCL
A=M-D
D=M // D=old ARG
@ARG
M=D // ARG=old ARG
@4
D=A
@LCL
A=M-D
D=M // D=old LCL
@LCL
M=D // LCL=old LCL
@R13
A=M // A=return address
0;JMP // jump to return address
(Class2.set)
@0
D=A
(Class2.set$CHECK_LOCAL_VAR_COUNT)
@Class2.set$LOCAL_VAR_COUNT_ZERO
D;JEQ
@SP
AM=M+1
A=A-1
M=0
D=D-1
@Class2.set$CHECK_LOCAL_VAR_COUNT
0;JMP
(Class2.set$LOCAL_VAR_COUNT_ZERO)
0
@0
D=A
@ARG
A=M
A=D+A
D=M
@SP
A=M
M=D
@SP
M=M+1
@SP
AM=M-1
D=M
@Class2.0
M=D
@1
D=A
@ARG
A=M
A=D+A
D=M
@SP
A=M
M=D
@SP
M=M+1
@SP
AM=M-1
D=M
@Class2.1
M=D
@0
D=A
@SP
A=M
M=D
@SP
M=M+1
@LCL
D=M // D=local base
@5
A=D-A // A=local base - 5(return address base)
D=M // D=return address
@R13
M=D // R13=return address
@SP
AM=M-1
D=M // D=return value
@ARG
A=M
M=D // arg[0]=return value
D=A+1
@SP
M=D // SP base=arg base + 1
@LCL
A=M-1
D=M // D=old THAT
@THAT
M=D // THAT=old THAT
@2
D=A
@LCL
A=M-D
D=M // D=old THIS
@THIS
M=D // THIS=old THIS
@3
D=A
@LCL
A=M-D
D=M // D=old ARG
@ARG
M=D // ARG=old ARG
@4
D=A
@LCL
A=M-D
D=M // D=old LCL
@LCL
M=D // LCL=old LCL
@R13
A=M // A=return address
0;JMP // jump to return address
(Class2.get)
@0
D=A
(Class2.get$CHECK_LOCAL_VAR_COUNT)
@Class2.get$LOCAL_VAR_COUNT_ZERO
D;JEQ
@SP
AM=M+1
A=A-1
M=0
D=D-1
@Class2.get$CHECK_LOCAL_VAR_COUNT
0;JMP
(Class2.get$LOCAL_VAR_COUNT_ZERO)
0
@Class2.0
D=M
@SP
A=M
M=D
@SP
M=M+1
@Class2.1
D=M
@SP
A=M
M=D
@SP
M=M+1
@SP
A=M
A=A-1
D=M
A=A-1
D=M-D
M=D
@SP
M=M-1
@LCL
D=M // D=local base
@5
A=D-A // A=local base - 5(return address base)
D=M // D=return address
@R13
M=D // R13=return address
@SP
AM=M-1
D=M // D=return value
@ARG
A=M
M=D // arg[0]=return value
D=A+1
@SP
M=D // SP base=arg base + 1
@LCL
A=M-1
D=M // D=old THAT
@THAT
M=D // THAT=old THAT
@2
D=A
@LCL
A=M-D
D=M // D=old THIS
@THIS
M=D // THIS=old THIS
@3
D=A
@LCL
A=M-D
D=M // D=old ARG
@ARG
M=D // ARG=old ARG
@4
D=A
@LCL
A=M-D
D=M // D=old LCL
@LCL
M=D // LCL=old LCL
@R13
A=M // A=return address
0;JMP // jump to return address
(Sys.init)
@0
D=A
(Sys.init$CHECK_LOCAL_VAR_COUNT)
@Sys.init$LOCAL_VAR_COUNT_ZERO
D;JEQ
@SP
AM=M+1
A=A-1
M=0
D=D-1
@Sys.init$CHECK_LOCAL_VAR_COUNT
0;JMP
(Sys.init$LOCAL_VAR_COUNT_ZERO)
0
@6
D=A
@SP
A=M
M=D
@SP
M=M+1
@8
D=A
@SP
A=M
M=D
@SP
M=M+1
@Class1.set$RETURN_ADDRESS_1
D=A
@SP
AM=M+1
A=A-1
M=D // top=return address
@LCL
D=M // D=local base
@SP
AM=M+1
A=A-1
M=D // top=local base
@ARG
D=M // D=arg base
@SP
AM=M+1
A=A-1
M=D // top=arg base
@THIS
D=M // D=this base
@SP
AM=M+1
A=A-1
M=D // top=this base
@THAT
D=M // D=that base
@SP
AM=M+1
A=A-1
M=D // top=that base
@2
D=A
@5
D=D+A // D=argCount + 5
@SP
D=M-D // D=stack base - argCount - 5
@ARG
M=D // arg base=stack base - argCount - 5
@SP
D=M
@LCL
M=D // local base=stack base
@Class1.set
0;JMP // jump to function entry
(Class1.set$RETURN_ADDRESS_1)
0
@SP
AM=M-1
D=M
@5
M=D
@23
D=A
@SP
A=M
M=D
@SP
M=M+1
@15
D=A
@SP
A=M
M=D
@SP
M=M+1
@Class2.set$RETURN_ADDRESS_2
D=A
@SP
AM=M+1
A=A-1
M=D // top=return address
@LCL
D=M // D=local base
@SP
AM=M+1
A=A-1
M=D // top=local base
@ARG
D=M // D=arg base
@SP
AM=M+1
A=A-1
M=D // top=arg base
@THIS
D=M // D=this base
@SP
AM=M+1
A=A-1
M=D // top=this base
@THAT
D=M // D=that base
@SP
AM=M+1
A=A-1
M=D // top=that base
@2
D=A
@5
D=D+A // D=argCount + 5
@SP
D=M-D // D=stack base - argCount - 5
@ARG
M=D // arg base=stack base - argCount - 5
@SP
D=M
@LCL
M=D // local base=stack base
@Class2.set
0;JMP // jump to function entry
(Class2.set$RETURN_ADDRESS_2)
0
@SP
AM=M-1
D=M
@5
M=D
@Class1.get$RETURN_ADDRESS_3
D=A
@SP
AM=M+1
A=A-1
M=D // top=return address
@LCL
D=M // D=local base
@SP
AM=M+1
A=A-1
M=D // top=local base
@ARG
D=M // D=arg base
@SP
AM=M+1
A=A-1
M=D // top=arg base
@THIS
D=M // D=this base
@SP
AM=M+1
A=A-1
M=D // top=this base
@THAT
D=M // D=that base
@SP
AM=M+1
A=A-1
M=D // top=that base
@0
D=A
@5
D=D+A // D=argCount + 5
@SP
D=M-D // D=stack base - argCount - 5
@ARG
M=D // arg base=stack base - argCount - 5
@SP
D=M
@LCL
M=D // local base=stack base
@Class1.get
0;JMP // jump to function entry
(Class1.get$RETURN_ADDRESS_3)
0
@Class2.get$RETURN_ADDRESS_4
D=A
@SP
AM=M+1
A=A-1
M=D // top=return address
@LCL
D=M // D=local base
@SP
AM=M+1
A=A-1
M=D // top=local base
@ARG
D=M // D=arg base
@SP
AM=M+1
A=A-1
M=D // top=arg base
@THIS
D=M // D=this base
@SP
AM=M+1
A=A-1
M=D // top=this base
@THAT
D=M // D=that base
@SP
AM=M+1
A=A-1
M=D // top=that base
@0
D=A
@5
D=D+A // D=argCount + 5
@SP
D=M-D // D=stack base - argCount - 5
@ARG
M=D // arg base=stack base - argCount - 5
@SP
D=M
@LCL
M=D // local base=stack base
@Class2.get
0;JMP // jump to function entry
(Class2.get$RETURN_ADDRESS_4)
0
(Sys.init$END)
@Sys.init$END
0;JMP